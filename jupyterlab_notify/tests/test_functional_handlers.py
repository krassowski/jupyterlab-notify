...
class TestNotifyHandler(AsyncHTTPTestCase):
    def get_app(self):
        ...
        return Application([
            (r"/api/jupyter-notify/notify", handlers.NotifyHandler, {"extension_app": self.dummy_app, "name": "test"}),
        ], **settings)
...
class TestNotifyTriggerHandler(AsyncHTTPTestCase):
    def get_app(self):
        ...
        return Application([
            (r"/api/jupyter-notify/notify-trigger", handlers.NotifyTriggerHandler, {"extension_app": self.dummy_app, "name":"test"}),
        ], **settings)
    ...
    def test_post_trigger(self):
        ...
        self.assertTrue(hasattr(self.dummy_app, "notification_sent") and self.dummy_app.notification_sent)
